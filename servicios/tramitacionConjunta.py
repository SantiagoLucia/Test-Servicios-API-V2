...
class TramitacionConjunta:
    ...
    def desvincularTramitacionConjunta(self, request):
        ...
        response = cliente.service.desvincularTramitacionConjunta(**request)
    ...
    def vincularTramitacionConjunta(self, request):
        token = self.obtenerToken()
        settings = Settings(extra_http_headers={'Authorization': 'Bearer ' + token}, raw_response=True)       
        cliente = Client(wsdl=self.url_servicio, settings=settings)
        t_ini = time.perf_counter()
        response = cliente.service.vincularTramitacionConjunta(**request)
        t_fin = time.perf_counter()
        codigo = response.status_code
        respuesta = response.content.decode('utf-8')
        t_total = t_fin - t_ini
