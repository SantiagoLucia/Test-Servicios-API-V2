...
class ConsultaRegistro:
    ...
    def consultarRegistroPorCUIT(self, request):
        ...
        response = cliente.service.consultarRegistroPorCUIT(**request)
    ...
    def consultarRegistroPorNumero(self, request):
        token = self.obtenerToken()
        settings = Settings(extra_http_headers={'Authorization': 'Bearer ' + token}, raw_response=True)       
        cliente = Client(wsdl=self.url_servicio, settings=settings)
        t_ini = time.perf_counter()
        response = cliente.service.consultarRegistroPorNumero(**request)
        t_fin = time.perf_counter()
        codigo = response.status_code
        respuesta = response.content.decode('utf-8')
        t_total = t_fin - t_ini
    ...
    def listarRegistroPublico(self, request):
        token = self.obtenerToken()
        settings = Settings(extra_http_headers={'Authorization': 'Bearer ' + token}, raw_response=True)       
        cliente = Client(wsdl=self.url_servicio, settings=settings)
        t_ini = time.perf_counter()
        response = cliente.service.listarRegistroPublico(**request)
        t_fin = time.perf_counter()
        codigo = response.status_code
        respuesta = response.content.decode('utf-8')
        t_total = t_fin - t_ini
    ...
    def listarTodosLosRegistrosPublicos(self, request):
        token = self.obtenerToken()
        settings = Settings(extra_http_headers={'Authorization': 'Bearer ' + token}, raw_response=True)       
        cliente = Client(wsdl=self.url_servicio, settings=settings)
        t_ini = time.perf_counter()
        response = cliente.service.listarTodosLosRegistrosPublicos(**request)
        t_fin = time.perf_counter()
        codigo = response.status_code
        respuesta = response.content.decode('utf-8')
        t_total = t_fin - t_ini
